          ...
          {#each soundscape.groups as group (group.id)}
            {#if local.some((entry) => entry.groupId === group.id)}
              ...
              {#each local.filter((entry) => entry.groupId === group.id) as definition (definition.id)}
                <button type="button" aria-pressed={selectedIds.includes(definition.id)} aria-label={selectedIds.includes(definition.id) && soundscape.persisted?.multipleEnabled ? t("music.soundscape.stop", definition.name) : selectedIds.includes(definition.id) && playing ? t("music.soundscape.pause") : t("music.soundscape.play", definition.name)} disabled={definition.availability !== "available" || soundscape.saving} class={selectedIds.includes(definition.id) ? "flex h-8 w-full items-center gap-2 rounded-lg bg-primary/10 px-2 text-left text-xs hover:bg-accent/70 disabled:opacity-40" : "flex h-8 w-full items-center gap-2 rounded-lg px-2 text-left text-xs hover:bg-accent/70 disabled:opacity-40"} onclick={() => { void soundscape.toggleSelection(definition.id); }}><span class="min-w-0 flex-1 truncate">{definition.name}</span>{#if definition.availability !== "available"}<span class="text-[0.65rem] text-muted-foreground">{t("music.soundscape.needsRepair")}</span>{/if}</button>
              {/each}
            {/if}
          {/each}
