  ...
  <div class="mx-auto grid min-h-full w-full max-w-2xl grid-rows-[minmax(4.5rem,1fr)_auto_minmax(4.5rem,1fr)] py-5 min-[760px]:py-8">
    ...
    <div class="flex flex-col gap-7">
      ...
      <div class="grid gap-5">
        ...
        {#if error}
          <p role="alert" class="flex items-start gap-2 text-sm leading-5 text-foreground">
            <CircleAlert size={16} strokeWidth={1.8} class="mt-0.5 shrink-0 text-muted-foreground" aria-hidden="true" />
            <span>{error}</span>
          </p>
        {/if}
      </div>
