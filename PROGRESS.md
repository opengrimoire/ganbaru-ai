...
| 2026-03-14 | Calendar: drag-to-create blocks (click+drag with 10-min grid snap), snap indicator line with time label, resize flip (drag past opposite edge), block hover highlight. Extracted shared drag controller (useDragController.svelte.ts) for WeekView/DayView. Fixed calendar time storage (local time, no UTC conversion). Scroll position preserved across view switches. Break overlay focus handling. Added vitest with 38 unit tests. Provisional DB reset button. Close shortcut Ctrl+Shift+W. Hour height 1.4x. |
