...
| 2026-03-13 | Pomodoro: added native pre-break notification (notify-rust, action buttons, transient, sound), fullscreen break overlay (GTK, countdown timer, space x3 dismiss, Esc x3 skip, 30s inactivity re-show via GNOME IdleMonitor). Keyboard shortcut blocking during overlay via gsettings (Super, Alt, dash-to-dock hotkeys). TIME_MULTIPLIER=1 for testing. |
