...
- After opening a PR, do not poll GitHub Actions repeatedly. If status is needed, wait once for about 5 minutes (for example with `sleep 300`), then check once. If the user reports checks are complete, check once immediately and report the result.
