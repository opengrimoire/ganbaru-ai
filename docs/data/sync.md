...
The non-owner keeps its last activated copy for read-only browsing. It refreshes from the current owner on application start, resume, LAN reconnect, or explicit user action without changing generation. While Android can reach the open desktop coordinator, it keeps a bounded authenticated long poll active so an explicit desktop request does not wait for the periodic reconnect interval. Disconnected retries retain the slower backoff. Android reloads its ordinary startup reconciliation after activation, which rebuilds Calendar notification schedules and republishes portable Doomscrolling rules. Already scheduled alarms and accepted rules continue while disconnected. Desktop Calendar changes cannot affect a force-stopped or disconnected phone until Android successfully refreshes.

Desktop and Android expose the linked relationship through a compact control beside Pomodoro. Its panel presents the current role and the existing refresh, ownership, retry, unlink, and recovery actions. A read-only replica does not add a permanent warning row to every application view. Whole-vault refresh remains an explicit or lifecycle operation and must not run after every mutation as a substitute for incremental synchronization.
