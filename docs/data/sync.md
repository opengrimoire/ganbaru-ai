...
The first linked-vault activation preserves the device's previous independent vault as described above. Android activation also retains a private rollback copy. Unlink does not silently choose a new owner. If the owning device is permanently lost, an explicit recovery action unlinks the devices and advances the local copy to a new standalone generation; the two copies do not merge.
