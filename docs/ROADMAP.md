...
- Local file playback: current loopback WebView fallback for basic local audio and video, plus GStreamer as the primary Rust-controlled playback engine target, FFmpeg/libav support through an LGPL-compatible build path, format detection, codec support, lower audio-only memory overhead, VLC-class transport latency, and native video surface embedding in the music view
