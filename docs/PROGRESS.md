...
- Run the v4 benchmark suite from the installed app and place the first canonical v4 rows in `docs/PERFORMANCE.md`.
...
- Benchmark harness v4: isolated benchmark DB phases, startup-boot, idle-memory, calendar-nav, event-panel-open, calendar-create-cancel, ics-import, and theme-editor-open scenarios; one Run all button; suite state persisted across restarts; question-oriented output that separates startup, memory, and latency records; and docs at `docs/features/performance-benchmark.md` plus canonical tracking rules in `docs/PERFORMANCE.md`.
