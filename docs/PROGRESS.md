...
- Theming: registry-based theme store with two built-in themes plus full user-theme CRUD (create, duplicate, edit, rename, delete, plus `replaceTheme` for in-place JSON edits). Layered token model: optional five-color `sources` palette (canvas, ink, primary, destructive, calCanvas) feeds a derivation engine that tints the rest of the shell automatically; a three-layer resolver (override, derived, base CSS) lets users opt into Quick colors for cohesive editing while still pinning individual tokens. Managed from a single Appearance settings page that lists every theme with per-row Apply, Duplicate, View or Edit, and Delete actions above the font and zoom controls. Opening a theme swaps the section into a detail view: built-ins show a read-only palette and a copy or save JSON panel; user themes show the full editor (grouped sections, paired bg/fg rows, Quick colors block with "Set up" and "Turn off" affordances, per-row Auto/Pinned provenance badges) with the in-house HSL color picker plus an editable JSON panel whose Apply validates and commits through `replaceTheme` (id locked). JSON import/export works through both clipboard and native file dialog (tauri-plugin-dialog).
