...
- Calendar core: week, day, and month views; numeric and letter calendar shortcuts; header controls and keyboard shortcuts for position-preserving internal calendar zoom; modified wheel input scrolls without zooming the timeline; single-step keyboard navigation with delayed held-arrow repeat; stable current-time-colored hover time guide over event blocks and while the event panel is open, including crossover resize edges; gesture-aware drag create, move, and resize; all-day events; recurring events with RFC 5545 RRULE support; timezone-aware persistence and rendering; edit sessions; undo/redo; pomodoro integration; and iCalendar import/export including `.ics.zip` imports.
- Calendar performance: slim in-memory event rows, heavy fields loaded on demand, window-bounded event expansion through `calendar-index.ts`, per-day buckets, shared positioned hour gridlines, lighter panel detail queries, hover/pointer prefetch, and parked/lazy EventPanel.
- Event panel stability: full detail preload before paint, no meeting-field pop-in, no control color animation on open, stable selected contours, safe create-panel outside-click handling, clickable discard-confirmation buttons, and reusable parked panel state without save/title regressions.
- App shell layout: compact, even edge spacing around the tabs and primary workspace, persisted right-click visibility controls for hideable title bar sections and compact tab labels, a `Ctrl + ,` settings shortcut, and a compact action-first help modal listing available keyboard shortcuts.
...
- Standard gate before reporting completion: `pnpm -w run validate`. After it passes, skip dev-server, Tauri launch, and HTTP smoke checks unless they are directly required.
