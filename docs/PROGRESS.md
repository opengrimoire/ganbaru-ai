...
- Theming: registry-based theme store with two built-in themes plus full user-theme CRUD (create, duplicate, edit, rename, delete, plus `replaceTheme` for in-place JSON edits). Layered token model: a five-color `sources` palette (canvas, ink, primary, destructive, calCanvas) feeds a derivation engine that tints the rest of the shell automatically; a three-layer resolver (override, derived, base CSS) keeps Quick colors cohesive while letting users pin individual tokens. Clone always synthesizes sources from the resolved palette so every new user theme opens in Quick-colors mode, with explicit overrides preserved as pinned tokens. Managed from a single Appearance settings page that lists every theme with per-row Apply, Duplicate, View or Edit, and Delete actions above the font and zoom controls. Opening a theme swaps the section into a detail view: built-ins show a read-only palette and a copy or save JSON panel; user themes show a quick-first editor where each source color heads a card of its driven tokens (Auto rows are a readonly swatch plus explicit Pin button; Pinned rows are a full ColorField plus unpin). The in-house HSL color picker drives every editable field and an editable JSON panel whose Apply validates and commits through `replaceTheme` (id locked). JSON import/export works through both clipboard and native file dialog (tauri-plugin-dialog).
