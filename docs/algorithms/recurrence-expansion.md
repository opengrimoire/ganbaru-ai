...
**DST transitions.** Times are stored in UTC, but recurrence "every day at 09:00 local" must respect local time across DST changes. The expander applies the recurrence in the template's timezone, then converts each instance back to UTC. This means a daily 09:00 event in `America/Los_Angeles` produces instances at the correct UTC offset before and after spring-forward/fall-back transitions, even though the UTC times shift by an hour.
