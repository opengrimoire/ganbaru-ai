...
Volume shows one exact percent value. Local playback supports `0%` to `150%`: values through `100%` use the media element volume, while `100%` to `150%` use a Web Audio gain stage. YouTube playback remains capped at `100%`. Clicking the volume icon toggles mute without moving the saved volume slider position.
