...
Local video files with non-zero playable timeline starts, such as some lossless cuts with edit lists, are normalized in the Music UI. The player seeks to the first playable frame automatically, reports that frame as `0:00`, and subtracts the skipped prefix from the displayed duration and seek bar.
