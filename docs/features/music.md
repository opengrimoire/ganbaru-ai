...
Native local audio now decodes through Rodio and Symphonia. Pause, seek, codec coverage, and memory use for local audio are controlled by the native plugin rather than the platform WebView media element. Local video remains a compatibility fallback and is still limited by the WebView media stack until a native video surface is implemented.
