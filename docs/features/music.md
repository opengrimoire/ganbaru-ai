...
Volume shows one exact percent value. Native local audio supports `0%` to `150%` through the Rust backend. Local video and YouTube are capped at `100%` and stay on their WebView media element volume paths. Routing videos through a Web Audio gain graph is intentionally avoided because it can bind playback to stale Linux Bluetooth outputs after device changes and is not worth the stability risk just to exceed normal video volume. When the OS reports an audio device change, active local video playback recreates the media element at the current position and resumes if it was playing. Scrolling over the Music view, including a visible video surface, adjusts volume. Clicking the volume percent text toggles mute without moving the saved volume slider position.
