...
**Local files.** Audio and video files on the user's disk. Current desktop builds validate local files through the Rust `plugins/media-player` boundary and play them in the WebView through a token-gated loopback media host on `127.0.0.1`. The host streams only registered local files and serves byte-range responses with media content types so WebView media playback can seek and buffer normally. This fallback supports the codecs available to the platform WebView.

The selected native playback direction is audio first. The native plugin now includes a Rodio backend with default features disabled and only playback plus the needed Symphonia decoding features enabled for common music formats. This keeps normal local music playback lighter than a full multimedia framework while still avoiding WebView media elements for audio. The visible Music view has not fully migrated to this backend yet, so the loopback WebView path remains the active compatibility path until the frontend migration is complete. GStreamer, libVLC, or an LGPL-compatible FFmpeg/libav path remain fallback candidates for local video, unsupported audio formats, or broader codec coverage. A build must not enable GPL or nonfree FFmpeg components unless the licensing impact is intentionally accepted.

The current local playback path is not VLC-class native playback yet. It does not decode through Rodio, Symphonia, FFmpeg, libav, GStreamer, or libVLC today. Pause, seek, codec coverage, and memory use are therefore limited by the platform WebView media element. Local volume and pause silence are controlled through a Web Audio gain stage when available, so the app can mute local playback immediately even if the WebView media pipeline is slower to finish pausing. The native backend target remains the path for consistently instant transport controls, lower overhead, and broader codec support.
