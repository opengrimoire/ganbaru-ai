...
- **Active events end before archive.** A running pomodoro event shows End event instead of delete or archive. Ending sets the event's end time to now and closes the run as event-expired/completed. Ending the only active pomodoro candidate uses the standard "Stop the focus session?" modal; ending one overlapping pomodoro event uses inline red confirmation. The user can then archive the resulting past event.
