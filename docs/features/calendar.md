...
New imports also store structured iCalendar object and component JSON in preservation tables. Projected events, attendees, alarms, and recurrence overrides link back to preserved components so full event loads can expose preservation status, projection warnings, and raw component JSON for export merging. The visible calendar window still reads only normalized event rows. Export merges linked preserved `VEVENT` and nested `VALARM` components with the app's current supported fields, so unsupported event properties and parameters survive supported edits. Rows without preserved components still export through generated components.
