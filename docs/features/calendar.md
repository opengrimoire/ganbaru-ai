...
- **Recurring delete scopes preserve protected occurrences.** `Only this` archives or deletes only the selected concrete occurrence. `Following` and `All` preview the affected occurrences uniformly, including past, present, and future visible occurrences. On commit, one semantic plan applies the final visible projection in one step, stops any affected active session after confirmation, and sends one atomic backend batch. Affected protected occurrences archive first, then unprotected future occurrences are removed from the visible series.
