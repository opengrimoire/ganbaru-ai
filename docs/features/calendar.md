...
New imports also store structured iCalendar object and component JSON in preservation tables. Projected events, attendees, alarms, and recurrence overrides link back to preserved components so full event loads can expose preservation status and projection warnings. The visible calendar window still reads only normalized event rows. Current export still serializes from projected calendar rows; preservation-backed export merging is a later compatibility milestone.
