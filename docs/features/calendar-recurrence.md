...
When a user edits a recurring event, the scope picker offers three choices: this, following, all. Each maps to a different structural operation.

### Detach ("edit this")

A single instance is pulled out of the series into a standalone event with a new UUID. The template gains an exception (EXDATE) for that date so the instance no longer expands from the rule.

All runs whose `event_id` matches the synthetic ID `templateId::date` are updated: their `event_id` and `original_event_id` move to the new standalone UUID. The standalone has no recurrence config of its own.
...
The template is capped with an UNTIL date set to the day before the selected instance. A new template is created starting from the selected instance with the updated properties and a fresh recurrence config.

Runs on past instances still reference the old template (which still exists, just capped). Runs on future instances will reference the new template's synthetic IDs.
...
The template's properties are updated directly. Past instances that already expanded will now expand with the new properties. This creates tension with invariant 7 (past events are never deleted), which is resolved by detaching all affected past instances first.
...
When to choose all: a change that the user wants applied to the future and is willing to disconnect the past from. The detach-past behavior keeps the past safe automatically.

## Recurring scope selector UX

The scope picker appears whenever the user saves edits to a recurring event. Default selection:
...
**Scope "this":** the active instance is detached into a standalone. The session transfers to the standalone's UUID. The recurrence change does not apply to the standalone. Simplest case.

**Scope "following":** the series splits. If the active instance is at the split point, the split moves to the next day; the active instance stays on the old template, which is capped at today. The session continues uninterrupted. If the active instance is before the split point, it is unaffected.
