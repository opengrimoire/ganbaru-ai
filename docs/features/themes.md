...
- **base:** `light` or `dark`. Carried only by `BuiltinTheme` (the code-pinned light and dark) and by preset definitions, where it picks the BASE token table the preset's sources are layered on top of. User themes do not carry it: the v2 snapshot already holds every resolved token, so any required fallback is computed from the snapshot's canvas luminance via `defaultIconLabelFromCanvas`.
