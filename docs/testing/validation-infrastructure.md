...
Android ceilings include the three theme modules and three Notes block modules behind their existing public APIs. These splits preserve theme editing, import/export, and the supported Notes block operations while separating their maintenance boundaries. The Music route allows 166 source modules for the playlist chooser, playback-order control, track preferences, and their shared helpers. The Android build allows 1,019 source modules overall. Route and total limits match the measured graph without extra headroom. Required mobile modules and forbidden platform imports remain enforced unchanged.
