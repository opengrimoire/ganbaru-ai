...
Android ceilings include the three theme modules and three Notes block modules behind their existing public APIs. These splits preserve theme editing, import/export, and the supported Notes block operations while separating their maintenance boundaries. The Music route allows 159 source modules so its playlist chooser can use the shared portal and viewport-aware popover positioning utilities. The production graph contains 1,008 source modules; excluding the six extracted files brings every other destination back within its previous ceiling. Route limits match the measured graph without extra headroom. Required mobile modules and forbidden platform imports remain enforced unchanged.
